    ...
    <div class="row">
        ...
        <div class="col-md-6">
            <table class="table">
              ...
              <tbody>
                @if($sectionsUser)
                <?php $cont=0; $compararId='jojo'; $fc = 1; ?>
                  @foreach($sectionsUser as $s)
                  <?php
                    if($s->id!=$compararId) {
                    if($cont==0) {$compararId=$s->id;
                    if($s->fecha<date('Y-m-d')) { $fc=0;} }
                    if($fc!=0) { ?>
                  <tr>
                      <td width="20%">{{$s->fecha}}</td>
                      <td class="text-jutify" width="30%"><?= substr($s->descripcion, 0, 80);?>...<a href="#" data-toggle="modal" data-target="#myModall{{$s->id}}"> Read more</a></td>
                      <td width="20%">{{$s->lugar}}</td>
                      <td width="30%">
                          <a href="#" class="btn btn-md btn-info" data-toggle="modal" data-target="#myModal{{$s->id}}">Details</a>
                          @if(Auth::user()->model != 'juridico')
                            <a href="#" class="btn btn-md btn-success" data-toggle="modal" data-target="#enrollP{{$s->id}}">Enroll</a>
                          @endif
                      </td>
                  </tr>
                  <?php } ?>
                      <!--  Read more -->
                      <div class="modal fade" id="myModall{{$s->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                         ...
                         <!-- Modal inscripcion -->
                      <div class="modal fade" id="enrollP{{$s->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                        <div class="modal-dialog" role="document">
                          <div class="modal-content">
                            <div class="modal-header">
                              <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                              <h4 class="modal-title" id="myModalLabel"><strong>{{$s->id}} - {{$servicio->nombre}} -  <spam style="color:red;">Service Cost: {{$s->costo}}</spam></strong></h4>
                            </div>
                        ...
                        <div class="modal-dialog" role="document">
                          <div class="modal-content">
                            <div class="modal-header">
                              <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                              <h4 class="modal-title" id="myModalLabel">Sections <strong>{{$s->id}} - {{$servicio->nombre}} -  <spam style="color:red;">Service Cost: {{$s->costo}}</spam></strong></h4>
                            </div>
                            <div class="modal-body">
                  ...
                  <?php $cont++;}
                  else{$compararId=$s->id;  if($s->fecha<date('Y-m-d')) { $fc=0;} else { $fc=1; } ?>
                ...
                <?php 
                if($sp->id!=$compararId) {
                    if($cont==0) {$compararId=$sp->id;
                    if($sp ->fecha<date('Y-m-d')) { $fc=0;} 
                  }
                    if($fc!=0) { ?>
                  <tr>
                      <td width="20%">{{$sp->fecha}}</td>
                      <td class="text-jutify" width="30%"><?= substr($sp->descripcion, 0, 80);?>...<a href="#" data-toggle="modal" data-target="#myModall{{$sp->id}}"> Read more</a></td>
                      <td width="20%">{{$sp->lugar}}</td>
                      <td width="30%">
                          <a href="#" class="btn btn-md btn-info" data-toggle="modal" data-target="#myModal{{$sp->id}}">Details</a>
                          @if(Auth::user()->model != 'juridico')
                  ...
                  <?php } ?>
                      <!--  Read more -->
                      <div class="modal fade" id="myModall{{$sp->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                          <div class="modal-dialog" role="document">
                            <div class="modal-content">
                              <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                      ...
                      <div class="modal fade" id="enrollP{{$sp->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                        <div class="modal-dialog" role="document">
                          <div class="modal-content">
                            <div class="modal-header">
                              <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                              <h4 class="modal-title" id="myModalLabel"><strong>{{$sp->id}} - {{$servicio->nombre}} -  <spam style="color:red;">Service Cost: {{$sp->costo}}</spam></strong></h4>
                            </div>
                            <div class="modal-body">
                              <div class="alert alert-danger hide" role="alert" id="message_error">
                        ...
                        <div class="modal-dialog" role="document">
                          <div class="modal-content">
                            <div class="modal-header">
                              <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                              <h4 class="modal-title" id="myModalLabel">Sections <strong>{{$sp->id}} - {{$servicio->nombre}} -  <spam style="color:red;">Service Cost: {{$sp->costo}}</spam></strong></h4>
                            </div>
                            <div class="modal-body">
                  ...
                  <?php $cont++;}else{$compararId=$sp->id;  if($sp->fecha<date('Y-m-d')) { $fc=0;} else { $fc=1; }?>
